...
class PreMatchPrediction:
    ...
    home_stake: str | None = None
    away_stake: str | None = None
...
_W_MOT_REL = 0.04   # relative motivation shift (home vs away)
_W_MOT_ABS = 0.03   # absolute draw suppression when both teams motivated


def _motivation_adjustments(
    home_motivation: float | None,
    away_motivation: float | None,
) -> tuple[float, float, float]:
    """Returns (home_adj, draw_adj, away_adj) from motivation."""
    if home_motivation is None or away_motivation is None:
        return 0.0, 0.0, 0.0
    rel = (home_motivation - away_motivation) * _W_MOT_REL
    total_mot = (home_motivation + away_motivation) / 2.0
    draw_adj = -(total_mot - 0.5) * _W_MOT_ABS
    return rel, draw_adj, -rel
...
def _composite_probs(
    ...
    mot_home_adj, mot_draw_adj, mot_away_adj = _motivation_adjustments(home_motivation, away_motivation)
    ...
    adj_away = (p_away_base
                ...
                - fatigue_away
                - injury_penalty_away)
    adj_draw = p_draw_base + (fatigue_home + fatigue_away) * 0.3 + mot_draw_adj
